...
PACMAN_ADDITIONAL_PKGS="liri-shell-git liri-wayland-git liri-workspace-git liri-settings-git liri-files-git liri-appcenter-git liri-terminal-git liri-wallpapers-git liri-themes-git xorg-server xorg-server-utils mesa-libgl phonon-qt5-gstreamer"
...
case "$arch" in
	armv*)
		...
		EXPECT_TIMEOUT=3600 # Most armv* based devices can be very slow (e.g. RPiv1)
		ARCH_KEYRING=archlinuxarm
		DOCKER_IMAGE_NAME="local/lirios/armv${version}h"
		;;
	*)
		...
		DOCKER_IMAGE_NAME="local/lirios/x86_64"
		;;
esac
...
arch-chroot $ROOTFS /bin/sh -c 'useradd -G wheel,video,input -ms /bin/bash lirios'
arch-chroot $ROOTFS /bin/sh -c 'echo "lirios:U6aMy0wojraho" | chpasswd -e'
arch-chroot $ROOTFS /bin/sh -c 'echo "lirios ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers'
